...
export class TimeLine extends DrawingElement<'TIME_LINE'> {
  ...
  drawElement = (zoom: number = 1, theme: Theme) => {
    ...
    let largeScale = 'black'
    let smallScale = 'rgba(128, 128, 128, 1)'
    ...
    if (this.allTime > 0) {
      ...
      this.ctx.strokeStyle = largeScale;
      ...
      for (let i = 0; i <= this.MAX_INTERVALS * zoom; i++) {
        ...
        for (let j = 1; j < 5; j++) {
          if (i < this.MAX_INTERVALS * zoom) {
            ...
            this.ctx.strokeStyle = smallScale;
