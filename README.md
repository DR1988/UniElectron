...
При запуске формы загружается демо-протокол (`initialState`, allTime = 350 с); **Reset** заменяет линии пустыми (`resetedState`), но сохраняет текст описания (draft-js) и имена пресетов. `allTime` пересчитывается как максимум endTime последнего изменения каждой линии (предполагается, что изменения отсортированы по времени).
