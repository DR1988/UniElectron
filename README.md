...
3. **Проверка «помещается ли»** — при движении по canvas проверяется каждая линия: свободный зазор от левого края превью до следующего ChangeElement должен быть не меньше длительности пресет-протокола (его `allTime`). Если на любой линии не хватает места, превью подсвечивается красным.
