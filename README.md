...
Редактирование времени валидируется: если интервал пересекает соседний, выставляется `crossingValueStart/End` и предупреждение (`wrongSign`) — сохранение заблокировано. Если endTime превышает `allTime`, показывается предупреждение «End time should not exceed All time. Change All time first.» и сохранение заблокировано — сначала нужно увеличить `allTime`. `allTime` задаётся пользователем полем «All time (sec)» под листом процесса (уменьшить её ниже максимального endTime по линиям нельзя).
...
При запуске формы загружается демо-протокол (`initialState`, allTime = 350 с); **Reset** заменяет линии пустыми (`resetedState`), но сохраняет текст описания (draft-js) и имена пресетов. `allTime` задаётся пользователем полем «All time (sec)» под листом процесса и не пересчитывается автоматически: endTime любого изменения не может превышать `allTime` (при попытке — предупреждение и блокировка сохранения), а уменьшить `allTime` ниже текущего максимального endTime тоже нельзя.
