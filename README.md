...
- **Графики** (recharts) — фактические RPM (красная линия) против уставок из протокола (зелёные ступени), зум/панорама кастомной шкалой (+/- и перетаскивание окна).
...
┌──────────────────────────── main process ────────────────────────────┐
│ app/main.ts                                                          │
│  • BrowserWindow (1400×1200, nodeIntegration + enableRemoteModule,   │
│    contextIsolation: off, backgroundThrottling: false)               │
...
│  Обе формы смонтированы одновременно, переключение — CSS show/hide   │
└──────────────────────────────────────────────────────────────────────┘
```
...
Обе формы (MainForm и Graphs) всегда смонтированы в `AppForms` одновременно; переключение между ними — только CSS show/hide, поэтому socket-подписки обеих форм активны постоянно. Кнопки переключателя форм (`FormChoserComponent`) закомментированы — фактически виден только MainForm.
...
│       │                       # Context.tsx — ThemeContext (светлая/тёмная тема)
...
- **Панель управления** — `Connect`, `Start` (неактивна во время проигрывания, до Stop/окончания протокола), `Stop` (неактивна без serial), `Reset` (сбрасывает линии в пустые, сохраняя описание и имена пресетов), `Save`/`Load` (диалоги выбора JSON-файла, заголовки на русском), тумблер `Close valves`/`Open valves` — открывает/закрывает HV1–HV3 разом (подпись показывает действие по клику). Кнопка Pause в UI закомментирована (событие PAUSE и метод контроллера при этом существуют).
- **Protocol Set Buttons** — 8 слотов пресетов: левый клик загружает сохранённый в localStorage протокол, правый клик открывает диалог выбора JSON-файла (имя файла без расширения становится именем слота). Хранятся в localStorage под ключами `firstTemporaryButton` … `eigthTemporaryButton` (опечатка в имени последнего ключа) в формате `{temporaryButtons: {buttonPosition, name}, protocol: {lineFormer, allTime, textEditorState}}`.
...
| Горизонтальное перетаскивание элемента (прямоугольника) | перемещение интервала по времени; коллизия с соседним элементом — красная подсветка и откат при отпускании; у краёв холста время ограничивается 0 / allTime |
...
Редактирование времени валидируется: если интервал пересекает соседний, выставляется `crossingValueStart/End` и предупреждение (`wrongSign`) — сохранение заблокировано. Если endTime превышает `allTime`, показывается предупреждение «End time should not exceed All time. Change All time first.» и сохранение заблокировано — сначала нужно увеличить `allTime`. `allTime` задаётся пользователем тремя полями «All time» (часы / минуты / секунды) под листом процесса; итог считается как `h·3600 + m·60 + s` секунд, минуты и секунды ограничены 0..59 (уменьшить её ниже максимального endTime по линиям нельзя).

### Схема реактора (SVG)

Схема показывает дозаторы (левый/правый), баллон инертного газа, реакционную камеру с мешалкой и все клапаны. Поверх всего SVG лежит прозрачный слой: клик в любом месте схемы открывает модалку ручного управления (поэлементные клики не реализованы). Подсветка элементов по текущему времени проигрывания присутствует в коде, но закомментирована — проп `time` передаётся компоненту, но не используется.
...
recharts LineChart (1200×300): красная линия фактических RPM (событие `rpmChange`, 1 раз в секунду во время проигрывания) и зелёные ступени уставок из протокола как ReferenceArea (строятся из изменений линии RPM при `start`). Ось Y — 0…3000. Зум/панорама — кастомная шкала под графиком: кнопки `+`/`-` (шаг 10 с) и перетаскивание окна; встроенный Brush recharts закомментирован. Есть кнопка `Clear` для очистки накопленных фактических значений. Событие `tempChange` подписано, но только логируется — график температуры не реализован.
...
- **Каждую секунду**:
  - эмитится `rpmChange` — значение `Serial.getRPMValue()` (сейчас всегда 0 — см. «Известные задачи»);
...
- **Конец** (`currentTime >= allTime`): отправка `S`, эмит `protocolFinish`, остановка таймера. Закрытие всех клапанов по окончании закомментировано в коде (остаются в последнем состоянии).
- **PAUSE** — останавливает таймер, UI сохраняет прогресс (`distance = 100 · currentTime / allTime`). Кнопки паузы в UI нет (закомментирована).
- **STOP** — очистка таймеров, отправка `S`, выключение термостата (если был включён) и повторная инициализация HID-устройства через 2 с (после перехода в off-состояние дескриптор HID недоступен). Реинициализация HID происходит безоговорочно — даже если термостат в протоколе не использовался.
- **Температура** — `startGettingTemperature()` (опрос каждые 5 с → событие `tempChange`) закомментирован в `start()`, поэтому во время проигрывания событие не эмитится.

## Аппаратная часть и протоколы обмена

### Serial-контроллер (Arduino/ATmega)

Подключение: `serialport`, **500000 baud**, parity none; входящие данные парсятся Readline-парсером (разделитель `\n\r`).
Поиск порта (`Connect`): перебираются последний и первый порты из `SerialPort.list()`, порт считается рабочим, если контроллер отвечает `CONNECTED` (таймаут ~3 с: опрос раз в секунду). Если порт уже подключён, повторный `Connect` просто подтверждает соединение.
...
Ответы контроллера: `CONNECTED`, коды состояния клапанов `C0C`/`C0O` … `C5C`/`C5O` (GV1–GV6), `CS0C`/`CS0O` … `CS2C`/`CS2O` (HV1–HV3) — ретранслируются в UI событием `VALVE_ACTION`.

> ⚠️ Фактические обороты мешалки из serial **не читаются**: метод `Serial.setRpmValue()` нигде не вызывается, поэтому `getRPMValue()` всегда возвращает 0. На графике «фактическая» линия RPM — ровная нулевая, а сравнение в `waitForValue` тоже происходит против 0.

Нюансы кодов состояний: в `serialMessages.ts` также объявлены коды `C6C`/`C6O`, `C7C`/`C7O`, но команда запроса запрашивает только C0–C5 и CS0–CS2, т.е. фактическое состояние V8 (HV3) опросить нельзя. В записи `'CS0C'` поле `name` содержит копи-пасту (`'C7O'`) — безвредно, в UI уходит реальный код из события.
...
Ищется среди HID-устройств по `manufacturer === 'TERMEX'`. Команды шлются в формате `:{serialNumber} <КОМАНДА>` как ASCII-массив (на Windows добавляется лидирующий байт `0x00`); ответы читаются синхронно (`readSync`) и разбираются разбиением по `0x00` — если `0x00` в ответе нет, считается StatusError:
...
> ⚠️ Известный дефект: `turnOn()` **безусловно** эмитит StatusError после успешного чтения ответа (лишний `emit` в коде). Из-за этого при старте любого протокола с TC-линией срабатывает ErrorHandler: UI на несколько секунд показывает «отключено», а через 3 с контроллер термостата пересоздаётся.
...
| `PAUSE` / `STOP` | — | пауза (кнопки в UI нет) / остановка |
...
| `rpmChange` | `{data, time}` | обороты мешалки из `getRPMValue()` (сейчас всегда 0) |
| `tempChange` | `{temperature, time}` | температура; **сейчас не эмитится** — `startGettingTemperature()` закомментирован |
...
Поля изменения (`Change`): `startTime`/`endTime` (сек), `changeId`, `duration`, `value` (обороты или температура, для RPM/TC), `waitForValue` (ожидание достижения значения RPM — недоработано), `crossingValueStart/End` (служебные: пересечение с соседним интервалом при редактировании), `idname` (служебное: `${название линии[0]}${id}`, например `V0`, `R9`, `T10`, `A11` — присваивается в main-процессе при старте проигрывания).

При запуске формы загружается демо-протокол (`initialState`, allTime = 350 с); **Reset** заменяет линии пустыми (`resetedState`), но сохраняет текст описания (draft-js) и имена пресетов. `allTime` задаётся пользователем тремя полями «All time» (часы / минуты / секунды) под листом процесса и не пересчитывается автоматически: итог считается как `h·3600 + m·60 + s` секунд, endTime любого изменения не может превышать `allTime` (при попытке — предупреждение и блокировка сохранения), а уменьшить `allTime` ниже текущего максимального endTime тоже нельзя.
...
6. График температуры не реализован: `tempChange` в Graphs только логируется; само событие при этом не эмитится (`startGettingTemperature()` закомментирован).
7. Фактические обороты мешалки не читаются из serial: `Serial.setRpmValue()` нигде не вызывается, `getRPMValue()` всегда 0 — «фактическая» линия на графике плоская, сравнение в `waitForValue` идёт против 0.
8. `ThermostatController.turnOn()` после успешного чтения ответа безусловно эмитит StatusError → при старте любого протокола с TC-линией срабатывает ErrorHandler (UI мигает «отключено», контроллер термостата пересоздаётся через 3 с).
9. Опрос состояния клапанов не покрывает V8 (HV3): команда запрашивает только C0–C5 и CS0–CS2; коды C6/C7 объявлены в `serialMessages.ts`, но не используются. В записи `'CS0C'` опечатка в поле `name` (`'C7O'`).
10. По окончании протокола отправляется только `S`; закрытие всех клапанов по завершении закомментировано.
11. Кнопки Pause и переключателя форм (MainForm/Graphs) закомментированы в UI — соответствующие возможности недоступны пользователю.
12. Подсветка элементов SVG-схемы по текущему времени проигрывания отключена (код закомментирован, проп `time` не используется).
