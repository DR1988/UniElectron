...
- **Протоколы** — сохранение/загрузка JSON-файлов (через диалоги Electron), 8 «быстрых» слотов пресетов (левый клик — загрузить; перенос на canvas — вставка в зазор между элементами с проверкой «помещается ли» и точной подстройкой времени; правый клик или кнопка «…» — выбрать файл; хранятся в localStorage), WYSIWYG-редактор описания протокола (draft-js).
...
│       │       ├── DnDProtocol/                # превью переносимого пресет-протокола (ghost над canvas)
...
│       │       │   │                                       # DnD пресетов (fit-проверка, модалка вставки)
...
- **Protocol Set Buttons** — 8 слотов пресетов: левый клик загружает сохранённый в localStorage протокол (полная замена текущего), перенос на canvas вставляет его в зазор между элементами (см. «Перенос пресет-протоколов»), правый клик или кнопка «…» справа в слоте открывает диалог выбора JSON-файла (имя файла без расширения становится именем слота). Хранятся в localStorage под ключами `firstTemporaryButton` … `eigthTemporaryButton` (опечатка в имени последнего ключа) в формате `{temporaryButtons: {buttonPosition, name}, protocol: {lineFormer, allTime, textEditorState}}`.
...
Редактирование времени валидируется: если интервал пересекает соседний, выставляется `crossingValueStart/End` и предупреждение (`wrongSign`) — сохранение заблокировано. Если endTime превышает `allTime`, показывается предупреждение «End time should not exceed All time. Change All time first.» и сохранение заблокировано — сначала нужно увеличить `allTime`. `allTime` задаётся пользователем тремя полями «All time» (часы / минуты / секунды) под листом процесса; итог считается как `h·3600 + m·60 + s` секунд. Ограничение минут/секунд 0..59 выполняется на вводе: значение больше 59 ввести нельзя, очищенное поле становится 0, ведущие нули отбрасываются («05» → «5»). Уменьшить `allTime` ниже максимального endTime по линиям нельзя.

### Перенос пресет-протоколов (вставка в зазор)

Протокол из слота Protocol Set Buttons можно вставить в текущий лист без замены:

1. **Захват** — удерживать кнопку слота ЛКМ. Превью появляется только при начале настоящего переноса (движение курсора > 5 px); простой клик загружает протокол полной заменой, без превью.
2. **Превью** — полупрозрачная копия протокола (`DnDProtocol`) центрируется на курсоре, следует за ним и масштабируется вместе с зумом canvas. Левый край превью соответствует потенциальной точке вставки.
3. **Проверка «помещается ли»** — при движении по canvas проверяется каждая линия: свободный зазор от левого края превью до следующего ChangeElement должен быть не меньше длительности пресет-протокола (его `allTime`). Если на любой линии не хватает места, превью подсвечивается красным; если помещается — элементы ChangeElement переносимого протокола в превью становятся зелёными.
4. **Дроп** — если протокол помещается, в точке броска открывается модалка с тремя полями часы/минуты/секунды (предзаполнены рассчитанным временем) для точной подстройки момента вставки. При каждом изменении проверка повторяется: если протокол больше не помещается, показывается «The protocol does not fit at this time» и кнопка OK блокируется. Пока модалка открыта, превью остаётся на canvas и двигается вместе с временем из полей; Cancel или клик вне окна отменяет операцию.
5. **Вставка** — OK вставляет изменения пресета в каждую линию на выбранное время (строки сопоставляются по `shortName`): ничего не сдвигается, `allTime` не увеличивается, вставленным элементам присваиваются новые уникальные `changeId`.
